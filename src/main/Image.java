...
public class Image {
	...
	public void invert() {
        ...
        public void emboss() {
            ...
            for(int i = 1; i < _rows; i++){
                    for(int j = 1; j < _cols; j++){
                        int value = maxDifference(_imageArray[i][j], _imageArray[i-1][j-1]);
                            _outputArray[i][j].red = value;
                            _outputArray[i][j].green = value;
                            _outputArray[i][j].blue = value;
                    }
        ...
        public int maxDifference(Pixel p1, Pixel p2){
            ...
            int maxDiff = diffRed;
            if(Math.abs(maxDiff) < Math.abs(diffGreen)) maxDiff = diffGreen;
            if(Math.abs(maxDiff) < Math.abs(diffBlue)) maxDiff = diffBlue;
            maxDiff += 128;
            if(maxDiff > 255) maxDiff = 255;
